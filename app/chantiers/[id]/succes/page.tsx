...
export default function SuccesPage() {
  ...
  useGSAP(() => {
    ...
    tl.fromTo(iconRef.current,
      { scale: 0, opacity: 0 },
      { scale: 1, opacity: 1, duration: 0.6, ease: 'back.out(1.7)' }, 0.2)
    tl.fromTo('.success-title',
      { opacity: 0, y: 16 },
      { opacity: 1, y: 0, duration: 0.5 }, 0.5)
    tl.fromTo('.success-sub',
      { opacity: 0, y: 10 },
  ...
  return (
    <MobileShell>
      <div ref={containerRef} style={{ width: '100%', minHeight: '100vh', background: '#fff',
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center',
          ...
          <div ref={iconRef} style={{ opacity: 0, position: 'relative' }}>
            <div style={{ width: 120, height: 120, borderRadius: '50%', background: '#EAF3DE',
              display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ position: 'absolute', inset: -8, borderRadius: '50%',
                border: `2px solid ${ACCENT}33`, animation: 'fadeIn .5s ease-out .4s both' }} />
          ...
          <div>
            <h1 className="success-title" style={{ fontSize: 28, fontWeight: 700, color: '#27500A',
              margin: 0, letterSpacing: -0.4, opacity: 0 }}>
              Chantier clôturé !
            </h1>
            <p className="success-sub" style={{ fontSize: 15, color: '#6B7280', margin: '8px 0 0', opacity: 0 }}>
              Tout est en ordre. Le procès-verbal est disponible.
            </p>
          </div>
            ...
            {items.map((it, i) => (
              <div key={i} style={{
                ...
                <div style={{ width: 24, height: 24, borderRadius: '50%',
                  background: step > i ? ACCENT : '#E5E7EB',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        {/* Buttons */}
        <div style={{ padding: '16px 16px 34px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {pdfUrl && (
            <a href={pdfUrl} target="_blank" rel="noreferrer" style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              height: 52, background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 12,
              color: '#15803D', fontWeight: 600, fontSize: 15, textDecoration: 'none',
              fontFamily: 'inherit', cursor: 'pointer',
            }}>
              <IconDownload size={18} sw={2} />
        ...
        <div style={{ position: 'absolute', bottom: 4, left: '50%', transform: 'translateX(-50%)',
          width: 134, height: 5, borderRadius: 100, background: 'rgba(0,0,0,0.2)',
