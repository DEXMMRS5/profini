    ...
    <MobileShell>
      <div ref={containerRef} style={{
        ...
        <div style={{
          ...
          borderBottom: '0.5px solid #E5E7EB', position: 'relative', zIndex: 2,
        }}>
        ...
        <div className="pf-scroll" style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 140px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 }}>
            <h2 style={{ fontSize: 18, fontWeight: 600, color: '#111827', margin: 0, letterSpacing: -0.2 }}>
              Chantiers actifs
            </h2>
...
function StatCard({ icon, value, label, accent }: { icon: React.ReactNode; value: string; label: string; accent: string }) {
  return (
    ...
    onMouseLeave={e => (e.currentTarget.style.transform = '')}>
      ...
      <div>
        <div style={{ fontSize: 22, fontWeight: 700, lineHeight: 1.1, color: '#111827' }}>{value}</div>
  ...
  return (
    ...
    onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, paddingTop: 10,
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontWeight: 600, color: '#111827' }}>
          <IconEuro size={14} sw={2} />{formatEur(c.montant_ttc)}
        </span>
        <span style={{ marginLeft: 'auto', color: '#6B7280' }}>
          <IconChevronRight size={16} sw={2} />
        </span>
      </div>
    </div>
  )
